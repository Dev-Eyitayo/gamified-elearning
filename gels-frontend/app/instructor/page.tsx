...
import { BrainCircuit, ShieldAlert, Activity, Users, Loader2, AlertCircle, ArrowUpRight, RotateCcw } from 'lucide-react';
...
export default function InstructorDashboard() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center font-sans text-slate-400">
        <Loader2 className="animate-spin mb-4 text-[#CE82FF]" size={48} strokeWidth={3} />
        <h2 className="font-black tracking-widest uppercase">Syncing Control Center...</h2>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-sans">
      ...
      {/* Header */}
      <div className="border-b-4 border-slate-200 pb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2 text-lg">Monitor real-time AI adaptations and cohort analytics.</p>
        </div>
        <button onClick={fetchDashboardData} className="bg-white text-slate-400 border-2 border-slate-200 hover:bg-slate-50 hover:text-slate-600 px-5 py-3 rounded-2xl font-black uppercase tracking-widest text-sm transition-all active:translate-y-1 hidden sm:block">
          Refresh Data
        </button>
      </div>
      ...
      <div className="bg-white border-2 border-slate-200 rounded-3xl overflow-hidden shadow-sm">
        <div className="p-6 border-b-2 border-slate-100 bg-slate-50 flex items-center gap-3">
          <ShieldAlert size={24} className="text-slate-400" strokeWidth={2.5} />
          <h2 className="font-black text-slate-600 uppercase tracking-widest text-sm">Glass-Box AI Action Log</h2>
        </div>
